...
public class MainCandleView extends ApplicationFrame {
    ...
    public static List<CandleStick> readDataFromCsv(String path) {
        try {
            List<String> lines = FileUtils.readLines(new File(path), Charset.forName("utf8"));
            List<CandleStick> candleStickList = new ArrayList<>(lines.size() - 1);
            ...
            for (String line : lines) {
                ...
                Date date = sourceSdf.parse(fieldValues[1]);
                CandleStick candleStick = new CandleStick(targetSdf.format(date),
                        Double.valueOf(fieldValues[2]), Double.valueOf(fieldValues[4]),
                        Double.valueOf(fieldValues[5]), Double.valueOf(fieldValues[3]),
                        Double.valueOf(fieldValues[3]));
                candleStick.setVolume(Double.valueOf(fieldValues[6]));
    ...
    public MainCandleView(final String title) throws Exception {
        super(title);

//        List<CandleStick> candleStickList = readDataFromCsv("/ROOT/data/601901.txt");
...
//            dateList[i] = targetSdf.parse(candleStick.getDate());
//            i++;
//        }
...
//                "601901", dateList, highList, lowList, openList, closeList, volumeList
//        );
    ...
    public static void main(String[] args) throws Exception {
        final MainCandleView demo = new MainCandleView("Candlestick Demo");
        demo.pack();
